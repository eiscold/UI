...
GearScoreFrame.showPlayer = 1
GearScoreFrame.showItem = 0
GearScoreFrame.itemTypes = {
	["INVTYPE_RELIC"] = { ["SlotMOD"] = .3164, ["ItemSlot"] = 18, ["Enchantable"] = false},
	["INVTYPE_TRINKET"] = { ["SlotMOD"] = .5625, ["ItemSlot"] = 33, ["Enchantable"] = false },
	...
	["INVTYPE_RANGED"] = { ["SlotMOD"] = .3164, ["ItemSlot"] = 18, ["Enchantable"] = true },
	["INVTYPE_THROWN"] = { ["SlotMOD"] = .3164, ["ItemSlot"] = 18, ["Enchantable"] = false },
	["INVTYPE_RANGEDRIGHT"] = { ["SlotMOD"] = .3164, ["ItemSlot"] = 18, ["Enchantable"] = false },
	...
	["INVTYPE_NECK"] = { ["SlotMOD"] = .5625, ["ItemSlot"] = 2, ["Enchantable"] = false },
	["INVTYPE_SHOULDER"] = { ["SlotMOD"] = .7500, ["ItemSlot"] = 3, ["Enchantable"] = true },
	["INVTYPE_CHEST"] = { ["SlotMOD"] = 1.0000, ["ItemSlot"] = 5, ["Enchantable"] = true },
	["INVTYPE_ROBE"] = { ["SlotMOD"] = 1.0000, ["ItemSlot"] = 5, ["Enchantable"] = true },
	["INVTYPE_WAIST"] = { ["SlotMOD"] = .7500, ["ItemSlot"] = 6, ["Enchantable"] = false },
	["INVTYPE_LEGS"] = { ["SlotMOD"] = 1.0000, ["ItemSlot"] = 7, ["Enchantable"] = true },
	["INVTYPE_FEET"] = { ["SlotMOD"] = .75, ["ItemSlot"] = 8, ["Enchantable"] = true },
	["INVTYPE_WRIST"] = { ["SlotMOD"] = .5625, ["ItemSlot"] = 9, ["Enchantable"] = true },
	["INVTYPE_HAND"] = { ["SlotMOD"] = .7500, ["ItemSlot"] = 10, ["Enchantable"] = true },
	["INVTYPE_FINGER"] = { ["SlotMOD"] = .5625, ["ItemSlot"] = 31, ["Enchantable"] = false },
	["INVTYPE_CLOAK"] = { ["SlotMOD"] = .5625, ["ItemSlot"] = 15, ["Enchantable"] = true },
	["INVTYPE_BODY"] = { ["SlotMOD"] = 0, ["ItemSlot"] = 4, ["Enchantable"] = false },
}
GearScoreFrame.formula = {
	["A"] = {
		[4] = { ["A"] = 91.45, ["B"] = .65 },
		[3] = { ["A"] = 81.375, ["B"] = .8125 },
		[2] = { ["A"] = 73, ["B"] = 1 }
...
function GetEnchantInfo(ItemLink, ItemEquipLoc)
	...
	if ItemSubString == "0" and GearScoreFrame.itemTypes[ItemEquipLoc]["Enchantable"] then
		 local percent = ( floor((-2 * ( GearScoreFrame.itemTypes[ItemEquipLoc]["SlotMOD"] )) * 100) / 100 )
...
function GetItemScore(ItemLink)
    ...
    if GearScoreFrame.itemTypes[ItemEquipLoc] then
        if ItemLevel > 120 then
			Table = GearScoreFrame.formula["A"]
		else
			Table = GearScoreFrame.formula["B"]
		end
		if ItemRarity >= 2 and ItemRarity <= 4 then
            local Red, Green, Blue = QualityColor((floor(((ItemLevel - Table[ItemRarity].A) / Table[ItemRarity].B) * 1 * Scale)) * 11.25 )
            GearScore = floor(((ItemLevel - Table[ItemRarity].A) / Table[ItemRarity].B) * GearScoreFrame.itemTypes[ItemEquipLoc].SlotMOD * Scale * QualityScale)
			...
			return GearScore, ItemLevel, GearScoreFrame.itemTypes[ItemEquipLoc].ItemSlot, Red, Green, Blue, PVPScore, ItemEquipLoc, percent
		end
  	end
...
function HookSetUnit(arg1, arg2)
	...
	if CanInspect"mouseover" and UnitName"mouseover" == Name and not InCombat then 
 	...
 	if MouseOverGearScore and MouseOverGearScore > 0 and GearScoreFrame.showPlayer == 1 then 
...
function HookItem(ItemName, ItemLink, Tooltip)
	...
	local ItemScore, ItemLevel, EquipLoc, Red, Green, Blue, PVPScore, ItemEquipLoc, enchantPercent = GetItemScore(ItemLink)
 	if ItemScore >= 0 then
		if GearScoreFrame.showItem == 1 then
			...
			if PCLASS == "HUNTER" then
				...
				if ItemEquipLoc == "INVTYPE_2HWEAPON" or ItemEquipLoc == "INVTYPE_WEAPONMAINHAND" or ItemEquipLoc == "INVTYPE_WEAPONOFFHAND" or ItemEquipLoc == "INVTYPE_WEAPON" or ItemEquipLoc == "INVTYPE_HOLDABLE" then
					Tooltip:AddLine("HunterScore: "..floor(ItemScore * .3164), Red, Green, Blue)
				end
			end
		end
	end
